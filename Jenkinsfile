pipeline{
    ...
    stages {
        ...
        stage("deploy project"){
            steps{
                sshPublisher(publishers: [sshPublisherDesc(configName: '192.168.74.137', transfers: [sshTransfer(cleanRemote: false, excludes: '', execCommand: 'echo "success"', execTimeout: 120000, flatten: false, makeEmptyDirs: false, noDefaultExcludes: false, patternSeparator: '[, ]+', remoteDirectory: '${remote_dir}/', remoteDirectorySDF: false, removePrefix: '', sourceFiles: '*.py')], usePromotionTimestamp: false, useWorkspaceInPromotion: false, verbose: false)])
            }
        }
    }
}
